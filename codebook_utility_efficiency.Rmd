...
This dataset merges utility-level performance metrics with country-level market structure and governance characteristics, as well as national electricity access rates. The data was cleaned, merged, and reshaped to support a benchmarking analysis of 49 electric utilities across 37 countries. The final tidy dataset includes 21 variables.

This codebook accompanies the dataset titled "Efficiency_Markets_Access.csv", which contains the cleaned and merged data used for analysis. The complete data wrangling process — including variable transformations, DEA score calculation, and data integration — is documented in the replication script "Efficiency-market-access_dataset_creation.R".
...
table(df$Ownership, useNA = "ifany") %>%
  as.data.frame() %>%
  kable(col.names = c("Ownership", "Count"), align = "c") %>%
...
table(df$Region, useNA = "ifany") %>%
  as.data.frame() %>%
  kable(col.names = c("Region", "Count"), align = "c") %>%
  kable_styling(full_width = FALSE)
```
...
table(df$`Income Group`, useNA = "ifany") %>%
  as.data.frame() %>%
  kable(col.names = c("Income Group", "Count"), align = "c") %>%
  kable_styling(full_width = FALSE)
```
...
table(df$`Market Structure`, useNA = "ifany") %>%
  as.data.frame() %>%
  kable(col.names = c("Market Structure", "Count"), align = "c") %>%
  kable_styling(full_width = FALSE)
```
...
table(df$Regulator, useNA = "ifany") %>%
  as.data.frame() %>%
  kable(col.names = c("Regulator", "Count"), align = "c") %>%
  kable_styling(full_width = FALSE)
```
...
table(df$`Private IPP Established`, useNA = "ifany") %>%
  as.data.frame() %>%
  kable(col.names = c("IPP Established", "Count"), align = "c") %>%
  kable_styling(full_width = FALSE)
```
...
table(df$Unbundling, useNA = "ifany") %>%
  as.data.frame() %>%
  kable(col.names = c("Unbundling", "Count"), align = "c") %>%
  kable_styling(full_width = FALSE)
```
...
table(df$`Out of VIU`, useNA = "ifany") %>%
  as.data.frame() %>%
  kable(col.names = c("Out of VIU", "Count"), align = "c") %>%
  kable_styling(full_width = FALSE)
```
